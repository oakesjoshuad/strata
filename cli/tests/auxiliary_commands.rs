...
fn capabilities_json_reports_environment_source_over_file_when_no_cli_flag() {
    ...
    // No CLI/env override was given for export_target, so it must still fall
    // through to the file tier -- this is the isolation that distinguishes
    // "env beats file" from "env beats everything for every setting".
    assert_eq!(
        capabilities["configuration"]["export_target"]["source"],
        "file"
    );
