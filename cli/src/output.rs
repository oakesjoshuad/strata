...
pub(crate) fn output_capabilities(json_flag: bool) -> Result<(), CliError> {
    let value = json!({"version": build_version(), "kinds": RecordKind::ALL, "commands": ["init", "new", "show", "search", "graph", "history", "link", "status", "revise", "validate", "schema", "capabilities", "relationships"], "json_output": ["new", "show", "search", "graph", "status", "revise", "validate", "schema", "capabilities", "relationships"]});
    emit_json(value, json_flag)
}
